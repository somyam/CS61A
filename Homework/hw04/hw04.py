...
#shift down to next set of trees
        return tree(label(t), [replace_leaf(rest, old,new) for rest in branches(t)])
...
def prune_leaves(t, vals):
    ...
    copied = [prune_leaves(b, vals) for b in branches(t)]

    two = [c for c in copied if c is not None]
...
def totals_tree(m):
    ...
    lefthandside = totals_tree(end(left(m)))
    righthandside = totals_tree(end(right(m)))

    return tree(total_weight(m), [lefthandside, righthandside])
