...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(evaluated, MacroProcedure):
            return scheme_eval(evaluated.apply_macro(rest, env), env)

        mapped = rest.map(lambda x: scheme_eval(x, env))

        return scheme_apply(evaluated, mapped, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        rest_paired = Pair(target.second, expressions.second)
        
        env.define(target.first, do_lambda_form(rest_paired, env))
...
def make_let_frame(bindings, env):
    ...
    child_formals, child_vals = nil, nil
    ...
    while bindings is not nil:
        ...
        child_formals = Pair(bindings.first.first, child_formals)
        child_vals = Pair(scheme_eval(bindings.first.second.first, env), child_vals)

        bindings = bindings.second


        check_formals(child_formals)

    return env.make_child_frame(child_formals, child_vals)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(first, Pair) and scheme_symbolp(first.first):

        env.define(first.first, MacroProcedure(first.second, expressions.second, env))

        return first.first
    else:

        wrong = first.first if isinstance(first, Pair) else first

        raise SchemeError('non-symbol: {0}'.format(wrong))
