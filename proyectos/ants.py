    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.is_container and insect.is_container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while places_away < self.min_range:
        	current_place = current_place.entrance
        	places_away += 1

        while current_place != hive:

            if places_away > self.max_range:
                return None

            if current_place.bees and places_away >= self.min_range and places_away <= self.max_range:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.armor = armor
        if QueenAnt.queen_present == False:
            QueenAnt.queen_present = True
            self.queen_ant = True
            self.armor = armor
        ...
        if self.imposter == False:
            ...
            while next_to != None:
                ...
                if next_to.ant:
                    ...
                    if next_to.ant not in QueenAnt.already_doubled:
                        next_to.ant.damage = next_to.ant.damage * 2
                        QueenAnt.already_doubled.append(next_to.ant)

                    if isinstance(next_to.ant, BodyguardAnt) and next_to.ant.contained_ant:
                        if next_to.ant.contained_ant not in QueenAnt.already_doubled:
                            next_to.ant.contained_ant.damage *= 2
                            QueenAnt.already_doubled.append(next_to.ant.contained_ant)

                next_to = next_to.exit
...
def apply_effect(effect, bee, duration):
    ...
    effected_action = effect(regular_action, bee)
    ...
    def do_it(colony):
        ...
        if duration > 0:
            duration -= 1
            effected_action(colony)

        else:
            regular_action(colony)
